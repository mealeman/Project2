...
CREATE TABLE IF NOT EXISTS `mydb`.`Contract` (
  ...
  PRIMARY KEY (`contractid`, `Pharmacy_name`, `PharmaceuticalCompany_Pharm Co name`),
  INDEX `fk_Contract_Pharmacy1_idx` (`Pharmacy_name` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`Pharmacy_sells_Drug` (
  ...
  PRIMARY KEY (`Pharmacy_name`, `drug_drug_id`, `drug_Pharmacy_sells_Drug_pharmacy_id`),
